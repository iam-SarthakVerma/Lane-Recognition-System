...
def slope_filter(lines, min_slope=0.35):
    """
    NYC footage has wider streets and shallower lane angles, so a slightly
    lower min_slope (0.35 vs 0.4) avoids discarding valid lane lines.
    """
    left, right = [], []
    if lines is None:
        return left, right
...
def fit_lane_line(points, y_top, y_bottom):
    ...
    try:
        ...
        return None
    return (int(np.polyval(poly, y_bottom)), y_bottom,
            int(np.polyval(poly, y_top)),    y_top)


class LaneSmoother:
    """Rolling-average smoother to eliminate frame-to-frame jitter."""
...
smoother = LaneSmoother(window=15)


def process(img):
    h, w = img.shape[:2]
    y_top    = int(0.62 * h)   # slightly lower horizon for urban/NYC footage
    y_bottom = h
    ...
    # Adaptive Canny — auto-calibrates to scene brightness
    ...
    # Proportional trapezoidal ROI (no hard-coded pixel values)
    ...
    roi_image = build_roi_mask(canny, roi_vertices)

    # Hough — tuned for urban scenes with shorter / broken lane markings
    lines = cv2.HoughLinesP(roi_image, 1, np.pi / 180,
                            threshold=25,
                            minLineLength=20,
                            maxLineGap=100)

    # Average all segments per lane side into one line
...
out    = cv2.VideoWriter("Manhattan_detection.avi", fourcc, fps,
...
while cap.isOpened():
    ...
    try:
        frame = process(frame)
        out.write(frame)
        cv2.imshow("NYC Lane Detection", frame)
